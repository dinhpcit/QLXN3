...
if ( $Request->GetInt( "save", "post", 0 ) == 1)
{
	...
	$data['maongbenhpham'] = $Request->GetString( "maongbenhpham", "post", '' );
	$data['manguoiduoclaymau'] = $Request->GetString( "manguoiduoclaymau", "post", '' );
	...
	/*elseif(empty($error)){
		$sql = "UPDATE `tbl_pro_kqxn` SET 
			...
			`maongbenhpham` =".$db->dbescape($data['maongbenhpham']).",
			`manguoiduoclaymau` =".$db->dbescape($data['manguoiduoclaymau']).",
