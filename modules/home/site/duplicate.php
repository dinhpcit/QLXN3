...
{
    $array_code = explode(",",$user_info['address']);
}
if ( $tinhthanh == "" || $tinhthanh == "undefined" )
{
    $i=1;
...
elseif ( $tinhthanh != "" && $tinhthanh != "undefined")
{
    $code_text = "'".$tinhthanh."'";
}
...
$array_where = array();
$array_where [] = " (`tinhthanh_code` IN (".$code_text." ) )";
...
while ( $row = $db->sql_fetchrow( $result, 2 ) )
{
	...
	$row['check'] = DF_BASE_SITEURL . "?" . DF_MODULE_VARIABLE . "=".$module."&" . DF_FUNCTION_VARIABLE . "=mcb&hoten=".$row['hovaten']."&tinhthanh=".$row['tinhthanh_code']."&ns=".$row['namsinh']."ckc=1";
...
if ( !empty($array_code) && count( $array_code) > 1 )
{
    foreach ( $array_tinhthanh as $row )
    {
        if( in_array($row['matinhthanh'],$array_code) )
        {
            $row['select'] = ($row['matinhthanh'] == $tinhthanh) ? "selected" : "";
            $xtpl->assign( 'ROW', $row );
            $xtpl->parse( 'main.tinhthanh.loop' );
        }
    }
