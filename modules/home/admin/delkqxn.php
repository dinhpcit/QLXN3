...
if ( $Request->GetInt( "save", "post", 0 ) == 1)
{
	if ( !empty($tenfile) ) 
	{
		$sql = "SELECT * FROM `tbl_bmxn` WHERE `bkfilepath` LIKE '%".$db->dblikeescape($tenfile)."%' ";
		$result = $db->sql_query( $sql );
		while ( $row = $db->sql_fetchrow( $result, 2 ) )
		{
			$sql_up = "UPDATE `tbl_bmxn` SET `bmaxn` = '', `bngayxetnghiem` = '0', `bphuongphapxetnghiem` = '', `bngaytrakqxn` = '0', `bdonviguimau` ='', `btinhtrangmau` = '', `bketquaxetnghiem` ='', `bctvalue` ='', `bkuptime` = '0', `bkuserid` = '0', `bkfilepath` = '' WHERE id =".$row['id'];	
			$db->sql_query( $sql_up );
		}
		$sql_dl = "DELETE FROM `tbl_kqxn` WHERE `kfilepath` LIKE '%".$db->dblikeescape($tenfile)."%' ";
		$db->sql_query( $sql_dl );	
