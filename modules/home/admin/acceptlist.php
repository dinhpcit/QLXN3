...
if( $ckd == "1" ) $array_where [] = "( `labcode` ='' OR `labcode` IS NULL )";
...
if ( $Request->GetInt( "save", "post", 0 ) == 1)
{
	...
	elseif ( ( !empty($bmangay) || !empty($tenfile)) && !empty($lab)) 
	{
		$sql_dl = "UPDATE `tbl_bmxn` SET `labcode` = ".$db->dbescape($lab)." ".$where;
		$db->sql_query( $sql_dl );	
...
$xtpl->assign( 'ckd'.$ckd, 'checked="checked"' );
...
if ( !empty($bmangay) || !empty($tenfile) ) 
{
	...
	$base_url = DF_BASE_ADMINURL . "?" . DF_MODULE_VARIABLE . "=".$module."&" . DF_FUNCTION_VARIABLE . "=acceptlist";
