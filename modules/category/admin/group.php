...
$base_url = DF_BASE_ADMINURL . "index.php?" . DF_MODULE_VARIABLE . "=".$module."&" . DF_FUNCTION_VARIABLE . '=group';
$i= ($page*$per_page) + 1;	
while ( $row = $db->sql_fetchrow( $result, 2 ) )
{
	...
	$row['ck'] = md5($row['id']);
	$row['edit'] = DF_BASE_ADMINURL . "?" . DF_MODULE_VARIABLE . "=".$module."&amp;" . DF_FUNCTION_VARIABLE . "=group&amp;id=".$row['id'].'&amp;ck='.$row['ck'];
	$xtpl->assign( 'ROW', $row );
...
if ( $Request->GetInt( "save", "post", 0 ) == 1)
{
	...
	if ( empty($data['title']) ) $error = "Error: Bạn chưa điền tiêu đề";
	...
	if( empty($error) )
	{
		if ( $id == 0 )
		{
			...
			{
				fn_delete_all_cache ();
				header( "Location: " . DF_BASE_ADMINURL . "?" . DF_MODULE_VARIABLE . "=".$module."&" . DF_FUNCTION_VARIABLE . "=group");
				die();
			}
			else
			{
...
$xtpl->assign( 'link_cdel', DF_BASE_ADMINURL . "?" . DF_MODULE_VARIABLE . "=".$module."&" . DF_FUNCTION_VARIABLE . "=group&del=" );
$xtpl->parse( 'main' );
