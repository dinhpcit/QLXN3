...
<table width="100%" style="border-bottom:#CCC 1px solid; padding:3px">
    <tbody>
        <tr>
			<td>
                <input type="button" onClick="go_approve()" value="Duyệt theo tích chọn" disabled="disabled"/>
                <input type="button" onClick="go_approveall()" value="Duyệt theo danh sách"/>
            </td>
    	</tr>
    </tbody>
</table>
        ...
        <table class="list">  
            <tr>
                <td align="left">
                  ...
                  &nbsp;&nbsp;&nbsp;&nbsp;
                  <button type="button" id="btduyet" onClick="duyet_ma()" class="btn btn-info startUpload">Xác nhận và duyệt (0 bản ghi)</button> 
...
function setvalue(valbn,ck)
{
	...
	$('#lid').val(txt_url);
	$('#btduyet').html("Xác nhận và duyệt ("+bn_array.length+" bản ghi)");
}
function duyet_ma()
{
	var txt_url = bn_array.join(",");
	if (bn_array.length == 0 ) { alert("Bạn chưa chọn bản ghi!!!"); return; }
	var r = confirm("Bạn có chắc chắn duyệt "+bn_array.length+" bản ghi?");
	if (r == true) 
	{
